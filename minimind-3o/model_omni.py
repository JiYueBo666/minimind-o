...
class MiniMindOmni(MiniMindForCausalLM):
    ...
    def __init__(self, config: OmniConfig = None, audio_encoder_path="./model/SenseVoiceSmall", vision_model_path=None):
    ...
    @staticmethod
    def load_sensevoice(path, device=None):
        ...
        try:
            with contextlib.redirect_stdout(io.StringIO()):
                ...
                m = AutoModel(model=path, trust_remote_code=True, disable_update=True, device=device or "cpu")
        except (NotImplementedError, RuntimeError, Exception) as e:
            warnings.warn(f"[MiniMindOmni] Failed to load SenseVoice during init (will be attached later): {e}")
            return None, None
    ...
    def load_vision(path):
        if path is not None:
            warnings.warn(f"[MiniMindOmni] Vision disabled — vision_model_path ignored. vision_encoder will be None.")
        return None, None
    ...
    def forward(self, input_ids, attention_mask=None, past_key_values=None, use_cache=False, logits_to_keep=0, audio_inputs=None, audio_lens=None, pixel_values=None, **args):
        ...
        skip_text_logits = args.get('skip_text_logits', False)
        text_logits = None if skip_text_logits else self.thinker.lm_head(h_thinker[:, slice_indices, :])
    ...
    def stream_generate(self, input_ids, eos_token_id, max_new_tokens, temperature, top_p, rp, use_cache, return_audio_codes=False, **args):
        ...
        voice_only = args.get('voice_only', False)
        while input_ids.shape[1] < start_pos + max_new_tokens:
            forward_args = {**args, 'skip_text_logits': text_finished}
            if past_kvs is None or not use_cache:
                out = self.forward(torch.cat((audio_buffer, input_ids.unsqueeze(1)), dim=1), past_key_values=past_kvs, use_cache=use_cache, **forward_args)
            else:
                out = self.forward(torch.cat((audio_buffer[:, :, -1:], input_ids[:, -1:].unsqueeze(1)), dim=1), past_key_values=past_kvs, use_cache=use_cache, **forward_args)
