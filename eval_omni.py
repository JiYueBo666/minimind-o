...
# --------- memory profiling ---------
def _mem_report(tag="", show_delta=False, show_peak=False):
    """统一的内存报告"""
    lines = [f"[{tag}]"]
    ...
    if torch.cuda.is_available():
        ...
        rsrv = torch.cuda.memory_reserved() / 1024**3
        peak = torch.cuda.max_memory_allocated() / 1024**3
        lines.append(f"VRAM={alloc:.3f}G")
        if show_peak:
            lines.append(f"peak={peak:.3f}G")
        if show_delta and _base_gpu > 0:
            lines.append(f"Δ={alloc - _base_gpu:+.3f}G")

    # RAM
    rss = _process.memory_info().rss / 1024**3
    lines.append(f"RAM={rss:.3f}G")
    if show_delta and _base_ram > 0:
        lines.append(f"Δ={rss - _base_ram:+.3f}G")
...
def init_model(args):
    # 基线测量
    global _base_gpu, _base_ram
    if args.profile_memory:
        torch.cuda.reset_peak_memory_stats()
        _base_gpu = torch.cuda.memory_allocated() / 1024**3
        _base_ram = _process.memory_info().rss / 1024**3
        _mem_report("baseline", show_peak=True)
    ...
    if "model" in args.load_from:
        ...
        model.audio_encoder, model.audio_processor = MiniMindOmni.load_sensevoice(
            "./model/SenseVoiceSmall"
        )
        model.vision_encoder, model.vision_processor = MiniMindOmni.load_vision(
    ...
    if args.profile_memory:
        _mem_report("after_init", show_delta=True, show_peak=True)
...
def eval_sample(
    ...
    profile = getattr(args, "profile_memory", False)
    if profile:
        _mem_report("pre_generate")
        torch.cuda.reset_peak_memory_stats()
    ...
    with torch.no_grad():
        ...
        if profile:
            _mem_report("post_generate", show_delta=True)
...
def main():
    ...
    if "2" in modes:
        ...
        print(f"audio files: {len(audio_files_en)} (en), {len(audio_files_zh)} (zh)")
