...
def decode_mimi(mimi_codes):
    ...
    ).to(next(mimi_model.parameters()).device)
    with torch.no_grad():
...
def load_hf_model(model_path):
    ...
    with model_lock:
        ...
        audio_encoder, audio_processor = MiniMindOmni.load_sensevoice(
            args.audio_encoder
        )
...
def chat_stream(
    ...
    audio_inputs, audio_lens, pixel_values, ref_codes, spk_emb = (
        None,
        None,
        None,
        None,
    ...
    if audio_input is not None:
        ...
        if sr != 16000:
            samples = librosa.resample(
                samples.astype(float), orig_sr=sr, target_sr=16000
            ).astype(np.float32)
        ...
        def _do_asr():
            r = asr_model.generate(
                input=samples_for_asr, cache={}, language="auto", use_itn=True
            )
            asr_result[0] = (
                rich_transcription_postprocess(r[0]["text"]).strip() if r else ""
            )
    ...
    if voice_name != "default" and voice_name in voices_data:
        ...
        spk_emb = (
            v["spk_emb"].half().unsqueeze(0).to(device) if "spk_emb" in v else None
        )
    ...
    x = torch.tensor(
        tokenizer(inputs_text).data["input_ids"], dtype=torch.long, device=device
    )[None, ...]
...
def launch_gradio(server_name="0.0.0.0", server_port=8888):
    ...
    for s in sorted(builtin_voices):
        voice_choices.append((f"[内置] {s}", s))
    for s in sorted(clone_voices):
        voice_choices.append((f"[克隆] {s}", s))
    ...
    def respond(message, audio, voice, chat_history, model_history, max_turns):
        ...
        if not text and audio is None and img_path is None:
            yield chat_history + [
                {"role": "assistant", "content": "请输入文本、上传图片或录制音频"}
        ...
        if audio is not None:
            sr, samples = audio
            display_samples = (
                samples.mean(axis=1) if len(samples.shape) > 1 else samples
            )
            wav_path = tempfile.NamedTemporaryFile(suffix=".wav", delete=False).name
            sf.write(wav_path, display_samples, sr)
            chat_history = chat_history + [
                {"role": "user", "content": {"path": wav_path}}
            ]
        elif img_path:
            chat_history = chat_history + [
                {"role": "user", "content": {"path": img_path}},
                {"role": "user", "content": text or "请描述这张图片"},
            ]
        else:
        ...
        with gr.Row(equal_height=True):
            ...
            with gr.Column(scale=4):
                msg = gr.MultimodalTextbox(
                    placeholder="输入文本", show_label=False, submit_btn="发送"
                )
        with gr.Row():
            ...
            if model_dict:
                ...
                status = gr.Textbox(
                    value=f"已加载: {current_model_name}",
                    label="状态",
                    interactive=False,
                    scale=2,
                )
                model_dd.change(
                    lambda n: load_hf_model(model_dict[n]), [model_dd], [status]
                )

        msg.submit(
            respond,
            [msg, aud, voice_dd, chatbot, model_history, turns_dd],
            [chatbot, audio_out, aud, model_history, audio_status],
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--load_from", default="./", type=str, help="transformers模型扫描目录"
    )
